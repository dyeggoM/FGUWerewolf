...
function onDesktopInit()
	if Session.IsHost then
		...
		DB.addHandler("partysheet.*.name", "onUpdate", PartyManager.updateName);
...
function onEntryDeleted(nodePS)
	...
	if aEntryMap[sRecordName] then
		DB.removeHandler(sRecordName, "onDelete", PartyManager.onEntryDeleted);
...
function linkRecordField(nodeRecord, nodePS, sField, sType, sPSField)
	...
	if not aEntryMap[nodePS.getPath()] then
		DB.addHandler(nodePS.getPath(), "onDelete", PartyManager.onEntryDeleted);
		aEntryMap[nodePS.getPath()] = true;
	end
	
	local nodeField = nodeRecord.createChild(sField, sType);
	DB.addHandler(nodeField.getPath(), 'onUpdate', PartyManager.onLinkUpdated);
	DB.addHandler(nodeField.getPath(), 'onDelete', PartyManager.onLinkDeleted);
...
function getNodeFromToken(token)
	...
	return PartyManager.getNodeFromTokenRef(nodeContainer, nID);
end
