...
function onDesktopInit()
	if not Session.IsHost then
		User.onIdentityActivation = LanguageManager.onIdentityActivation;
		User.onIdentityStateChange = LanguageManager.onIdentityStateChange;
	end
...
function setSpeakerIdentity(sIdentity)
	...
	if (sActiveIdentity or "") ~= "" then
		DB.removeHandler("charsheet." .. sActiveIdentity .. "." .. CHAR_LANGUAGE_LIST, "onChildDeleted", LanguageManager.refreshChatLanguages);
		DB.removeHandler("charsheet." .. sActiveIdentity .. "." .. CHAR_LANGUAGE_LIST .. ".*." .. CHAR_LANGUAGE_LIST_NAME, "onUpdate", LanguageManager.refreshChatLanguages);
	end
	
	if (sIdentity or "") ~= "" then
		DB.addHandler("charsheet." .. sIdentity .. "." .. CHAR_LANGUAGE_LIST, "onChildDeleted", LanguageManager.refreshChatLanguages);
		DB.addHandler("charsheet." .. sIdentity .. "." .. CHAR_LANGUAGE_LIST .. ".*." .. CHAR_LANGUAGE_LIST_NAME, "onUpdate", LanguageManager.refreshChatLanguages);
	end
	...
	if bHost then
		...
		if nodeChar then
			for _,v in pairs(DB.getChildren(nodeChar, CHAR_LANGUAGE_LIST)) do
				...
				if (sLang or "") ~= "" then
					if LanguageManager.aLanguagesTongues[sLangLower] then
						bSpeaksAll = true;
						bUnderstandsAll = true;
					elseif LanguageManager.aLanguageSpeaksAll[sLangLower] then
						bSpeaksAll = true;
					elseif LanguageManager.aLanguagesUnderstandsAll[sLangLower] then
						bUnderstandsAll = true;
					elseif LanguageManager.aCampaignLangLower[sLangLower] then
