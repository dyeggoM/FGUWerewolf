...
function onDesktopInit()
	User.onUserStateChange = CharacterListManager.onUserStateChange;
	User.onIdentityActivation = CharacterListManager.onIdentityActivation;
	User.onIdentityStateChange = CharacterListManager.onIdentityStateChange;
end
...
function registerWindow(w)
	...
	if _wCharList and _wCharList then
		_wCharList.anchor.setStaticBounds(CharacterListManager.LEFT_MARGIN - CharacterListManager.PORTRAIT_PADDING, CharacterListManager.TOP_MARGIN, 0, 0);
	end
	if _wCharList then
...
function layoutControls()
	...
	for k, v in pairs(User.getAllActiveIdentities()) do
		table.insert(tIdentityList, { name = v, control = CharacterListManager.findControlForIdentity(v) });
	end
...
function resizeWindow()
	...
	local x = CharacterListManager.LEFT_MARGIN + CharacterListManager.RIGHT_MARGIN;
	local y = CharacterListManager.TOP_MARGIN + CharacterListManager.PORTRAIT_SIZE + CharacterListManager.BOTTOM_MARGIN;
	local count = CharacterListManager.getEntryCount();
	if count > 0 then
		x = x + (count * CharacterListManager.PORTRAIT_SIZE) + ((count - 1) * CharacterListManager.PORTRAIT_PADDING);
	end
...
function createEntry(sIdentity)
	...
	-- Configure control
	ctrlChar.setAnchor("top", nil, "top", "absolute", CharacterListManager.TOP_MARGIN);
	ctrlChar.setAnchor("left", "anchor", "right", "relative", CharacterListManager.PORTRAIT_PADDING);
	ctrlChar.setAnchoredWidth(CharacterListManager.PORTRAIT_SIZE);
	ctrlChar.setAnchoredHeight(CharacterListManager.PORTRAIT_SIZE);
...
function onDefaultDrop(sIdentity, draginfo)
	return CombatDropManager.handleAnyDrop(draginfo, "charsheet." .. sIdentity);
end
